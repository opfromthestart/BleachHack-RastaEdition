...
public class CmdDupe extends Command {
	...
	public void onCommand(String alias, String[] args) throws Exception {
		...
		if (args[0].equalsIgnoreCase("old")) {
			...
			listTag.add(0, builder1.toString());
			...
			for(int i = 1; i < 40; i++)
				listTag.add(i, string2);

			ItemStack bookStack = new ItemStack(Items.WRITABLE_BOOK, 1);
			bookStack.setSubNbt("title", NbtString.of("If you can see this, it didn't work"));
			//bookStack.setSubNbt("pages", listTag);

			mc.player.networkHandler.sendPacket(new BookUpdateC2SPacket(mc.player.getInventory().selectedSlot, listTag, Optional.of("If you can see this, it didn't work")));
		} else {
			throw new CmdSyntaxException("Invaild dupe method");
		}
	}
